...
import { X, Search, MapPin, DollarSign, Clock, Building } from 'lucide-react';
...
export const AdvancedJobFilters = ({ onApplyFilters, initialFilters }: AdvancedJobFiltersProps) => {
  ...
  const applyFilters = () => {
    ...
    let processedSearch = filters.search;
    if (advancedSearch) {
      processedSearch = advancedSearch;
    }
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          <Label htmlFor="advanced-search">Advanced Search (use AND, OR, NOT)</Label>
          <Input
            id="advanced-search"
            value={advancedSearch}
            onChange={(e) => setAdvancedSearch(e.target.value)}
            placeholder='e.g., "Software Engineer" AND (React OR Vue) NOT Intern'
          />
          <p className="text-xs text-gray-500 mt-1">
            Use quotes for exact phrases, AND/OR/NOT for operators
          </p>
        </div>
